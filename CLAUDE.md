...
19. `pvp_powerup_cost` — zero-dependency stardust + candy summation over a half-level climb. The canonical pre-XL table (L1 → L40, 78 half-level steps, built from 20 buckets of 4 steps except the final L39-L40 pair) is hardcoded in `powerupStepTable`. Stable since Pokémon GO launch — L1→L40 always costs exactly 270,000 stardust + 350 candy. Levels above L40 are explicitly rejected with `ErrXLRangeNotSupported`: the XL-candy era has shifted across Niantic adjustments and the simulator does not emit those values rather than risk staleness. Levels off the 0.5 grid are rejected with `ErrInvalidLevel`. Two availability flags signal missing upstream data; a zero cost without the flag is never authoritative. The `Note` composes shadow and availability caveats — they are not mutually exclusive on partial data.
