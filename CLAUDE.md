...
Phase X-II landed: `CombatantOptions` extended to info-path tools (`pvp_rank`, `pvp_species_info`, `pvp_level_from_cp`, `pvp_cp_limits`, `pvp_evolution_preview`, `pvp_rank_batch`). Every one of them drives species lookup through `resolveSpeciesLookup` + accepts `Options.Shadow` / `Lucky` / `Purified`. Lucky / Purified are no-op on info-path tools — stat and CP math are shadow/purified-independent — but accepted for struct-level symmetry across the whole MCP surface. The old `_shadow` suffix convention still works (dual-convention tolerance).
19. `pvp_powerup_cost` — zero-dependency stardust summation over a half-level climb. The canonical pre-XL stardust table (L1 → L40, 78 half-level steps, built from 20 buckets in `powerupStardustTable`) is hardcoded. L1 → L40 always costs exactly 270,000 stardust. Candy cost is deliberately NOT returned — public sources disagree on the per-half-step candy table above L20; the response carries a fixed `Note` string directing callers to Bulbapedia / GamePress / pvpoke for the candy number. Levels above L40 rejected with `ErrXLRangeNotSupported` (XL-candy era has shifted across Niantic adjustments); off-grid levels rejected with `ErrInvalidLevel`. Response shape is deterministic: `{from_level, to_level, steps, stardust_cost, note}` — no availability flags, no shadow handling (powerup cost is species-independent).
