...
18. `pvp_second_move_cost` — read-only lookup of `Species.ThirdMoveCost` (surfaced by the Phase 2C engine extension). pvpoke stores one numeric field and clients duplicate it as stardust + candy — mirrored in the response as `StardustCost` + `CandyCost` with equal values. Zero cost → `Available=false` with a `Note`.
