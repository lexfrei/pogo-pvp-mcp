...
Phase X-II follow-up: extend `Combatant.Options` pattern to info-path tools (`pvp_rank`, `pvp_species_info`, `pvp_level_from_cp`, `pvp_cp_limits`, `pvp_evolution_preview`, `pvp_rank_batch`). Today they still accept `_shadow` suffix directly; that convention is being phased out.
