...
20. `pvp_report_data_issue` — zero-dependency static-response tool. Takes no parameters, returns `{Message, RepositoryURL, IssuesURL, ChecklistHints}`. RepositoryURL / IssuesURL are hardcoded to `https://github.com/lexfrei/pogo-pvp-mcp` + `/issues/new`. Message explains the rationale (several tools carry hardcoded Niantic tables that drift between reference date and live game), ChecklistHints enumerates the structured info a good bug report should carry (tool name, exact input, observed output, expected output with source citation, observation date). No categorisation enum — that would rot as new classes of data drift appear and the human issue triage is quick enough that structured categories add no value.
