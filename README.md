...
**Status**: approaching v0.1. Fifteen MCP tools plus a `diff-gm` CLI helper are implemented:
...
- `pvp_weather_boost` — static lookup of Pokémon GO's weather → boosted-types table. Pass an empty `weather` to get all seven conditions (sunny / rainy / partly_cloudy / cloudy / windy / snow / fog) and their boosted types, or a specific name for one row. Carries the flat 1.1× multiplier.
...
Restart Claude Desktop. The fifteen `pvp_*` tools will appear in the tool list. If a tool returns "gamemaster not loaded", run `pogo-pvp-mcp fetch-gm` once to warm the cache.
