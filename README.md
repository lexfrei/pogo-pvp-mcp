...
- `pvp_second_move_cost` — per-species stardust + candy cost to unlock a second charged move. Stardust from gamemaster `thirdMoveCost`; candy derived from buddy distance (1km → 25, 3km → 50, 5km → 75, 20km → 100). Shadow species (`_shadow` suffix) pay `1.2×` both currencies (purified apply the inverse `0.8×` client-side). `stardust_cost_available` / `candy_cost_available` flags signal missing upstream data; zero costs without the flag are never a valid Pokémon GO value.
