...
- `pvp_evolution_target` — reverse-lookup for powerup planning: given the DESIRED evolved species and a league, walk `PreEvolution` back to the chain root and report the maximum CP of a wild root-species catch such that at least one IV spread evolves up to a target whose stat product clears `target_percent_of_best` (default 95) of the best legal spread under the league cap. Returns `from_species` (chain root), `chain_from_to` (root → ... → target), `max_cp_to_catch`, `max_level`, `typical_wild_cp_range` ([0/0/0 at L1, 15/15/15 at L30] for unboosted wild spawns), `percent_of_best_at_max`, `best_stat_product`, and a terse `evolution_hint` pointing callers at `pvp_evolution_cost` for per-step candy / item data. Rejects target species without a `PreEvolution` with `ErrNotInEvolutionChain` (catching a terminal species directly is a caller mistake). When the gamemaster snapshot is missing an ancestor species (drift between cached snapshot and current pvpoke data) the walk terminates at the last known species; the caller can detect the truncation by observing `from_species == target_species`. Shadow-aware lookup honours `options.shadow=true`. `cup` accepted for API symmetry but not currently enforced — the IV / stat-product math is cup-agnostic.
...
Restart Claude Desktop. The twenty-one `pvp_*` tools will appear in the tool list. If a tool returns "gamemaster not loaded", run `pogo-pvp-mcp fetch-gm` once to warm the cache.
