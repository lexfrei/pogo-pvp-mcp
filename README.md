...
- `pvp_powerup_cost` — sum stardust over a pre-XL (L1-L40) powerup climb in 0.5-level steps. L1 → L40 = 78 steps = 270,000 stardust. Candy is intentionally not returned — the per-half-step candy table has staggered boundaries that disagree across publicly-cited sources, so the tool refuses to guess and directs callers to consult Bulbapedia / GamePress / pvpoke. Levels above L40 (XL-candy era) rejected with `ErrXLRangeNotSupported`. `stardust_cost_available` / `candy_cost_available` flags signal missing upstream data; zero costs without the flag are never a valid Pokémon GO value.
