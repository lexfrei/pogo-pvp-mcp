...
// buildWiredServer stands up a fully-wired MCP server with all twenty
...
func buildWiredServer(t *testing.T) *mcp.Server {
	...
	metaTool := tools.NewMetaTool(ranks, mgr)
	...
	levelFromCPTool := tools.NewLevelFromCPTool(mgr)
	mcp.AddTool(mcpServer, levelFromCPTool.Tool(), levelFromCPTool.Handler())
	...
	rankBatchTool := tools.NewRankBatchTool(mgr, ranks)
	mcp.AddTool(mcpServer, rankBatchTool.Tool(), rankBatchTool.Handler())

	threatCoverageTool := tools.NewThreatCoverageTool(mgr, ranks)
	mcp.AddTool(mcpServer, threatCoverageTool.Tool(), threatCoverageTool.Handler())

	weatherBoostTool := tools.NewWeatherBoostTool()
	mcp.AddTool(mcpServer, weatherBoostTool.Tool(), weatherBoostTool.Handler())

	encounterCPRangeTool := tools.NewEncounterCPRangeTool(mgr)
	mcp.AddTool(mcpServer, encounterCPRangeTool.Tool(), encounterCPRangeTool.Handler())

	cupRulesTool := tools.NewCupRulesTool(mgr)
	mcp.AddTool(mcpServer, cupRulesTool.Tool(), cupRulesTool.Handler())

	secondMoveCostTool := tools.NewSecondMoveCostTool(mgr)
	mcp.AddTool(mcpServer, secondMoveCostTool.Tool(), secondMoveCostTool.Handler())

	powerupCostTool := tools.NewPowerupCostTool()
	mcp.AddTool(mcpServer, powerupCostTool.Tool(), powerupCostTool.Handler())

	reportDataIssueTool := tools.NewReportDataIssueTool()
	mcp.AddTool(mcpServer, reportDataIssueTool.Tool(), reportDataIssueTool.Handler())

	return mcpServer
}

// TestIntegration_ListTools verifies that a client connected via the
// in-memory transport sees all twenty currently implemented tools
