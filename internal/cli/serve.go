...
// buildMCPServer constructs the mcp.Server with all twenty-two currently
...
// pvp_encounter_cp_range, pvp_cup_rules, pvp_second_move_cost, pvp_powerup_cost,
// pvp_report_data_issue, pvp_pokedex_lookup, pvp_evolution_target).
...
func registerInfoAndLookupTools(
	...
	cupRulesTool := tools.NewCupRulesTool(gamemasterMgr)
	mcp.AddTool(server, cupRulesTool.Tool(), cupRulesTool.Handler())

	secondMoveCostTool := tools.NewSecondMoveCostTool(gamemasterMgr)
	mcp.AddTool(server, secondMoveCostTool.Tool(), secondMoveCostTool.Handler())

	powerupCostTool := tools.NewPowerupCostTool()
	mcp.AddTool(server, powerupCostTool.Tool(), powerupCostTool.Handler())

	reportDataIssueTool := tools.NewReportDataIssueTool()
	mcp.AddTool(server, reportDataIssueTool.Tool(), reportDataIssueTool.Handler())

	pokedexLookupTool := tools.NewPokedexLookupTool(gamemasterMgr)
	mcp.AddTool(server, pokedexLookupTool.Tool(), pokedexLookupTool.Handler())

	evolutionTargetTool := tools.NewEvolutionTargetTool(gamemasterMgr)
	mcp.AddTool(server, evolutionTargetTool.Tool(), evolutionTargetTool.Handler())
}
